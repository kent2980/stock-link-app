...
interface Disclosure33Props {
  code_33?: number | null;
  select_date?: string | null;
}

export default function Disclosure33(props: Disclosure33Props) {
  // コード33をpropsから取得
  const { code_33 } = props;
  ...
  const {
    ...
    queryKey: ["disclosureItems33", code_33],
    queryFn: async ({ pageParam = 1 }) => {
      return await FinancialSummaryService.getDisclosureItems({
        page: pageParam,
        limit: 20,
        code33: code_33 ? (code_33 === 0 ? undefined : code_33) : undefined,
        isDistinct: false,
        selectDate: props.select_date ?? null,
      });
    },
