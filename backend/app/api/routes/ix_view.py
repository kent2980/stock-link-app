...
@router.get(
    "/summary/items/simple/{xbrl_id}",
    response_model=sc.SimpleSummaryItems,
)
def read_summary_items_simple(
    *,
    session: SessionDep,
    xbrl_id: str,  # XBRLファイルのID
) -> sc.SimpleSummaryItems:
    """XBRLファイルのIDから決算情報を取得する"""

    statement = select(IxNonFraction).where(IxNonFraction.xbrl_id == xbrl_id)
    result = session.exec(statement).all()
    item = ssc.ix_non_fraction.IxNonFractionsPublic(data=result, count=len(result))
    PriorResultNumericItems = sc.SimpleSummaryNumericItems(
        items=item, year="Prior", member="Result"
    )
    CurrentResultNumericItems = sc.SimpleSummaryNumericItems(
        items=item, year="Current", member="Result"
    )
    CurrentForecastNumericItems = sc.SimpleSummaryNumericItems(
        items=item, year="Current", member="Forecast"
    )
    NextForecastNumericItems = sc.SimpleSummaryNumericItems(
        items=item, year="Next", member="Forecast"
    )

    statement = select(IxNonNumeric).where(IxNonNumeric.xbrl_id == xbrl_id)
    result = session.exec(statement).all()
    item = ssc.ix_non_numeric.IxNonNumericsPublic(data=result, count=len(result))
    FractionItems = sc.SimpleSummaryFractionItems(item)

    items = sc.SimpleSummaryItems(
        FractionItems=FractionItems,
        PriorResultNumericItems=PriorResultNumericItems,
        CurrentResultNumericItems=CurrentResultNumericItems,
        CurrentForecastNumericItems=CurrentForecastNumericItems,
        NextForecastNumericItems=NextForecastNumericItems,
    )

    return items


@router.get("/summary/items/simple/", response_model=sc.SimpleSummaryItems)
def read_summary_items_simple_is_code(
    *,
    session: SessionDep,
    code: str = Query(...),  # 銘柄コード
    count: int = Query(0),  # 期間
    period: Optional[str] = Query(None),  # 期間
) -> sc.SimpleSummaryItems:
    """銘柄コードから決算情報を取得する"""

    statement = (
        select(IxHeadTitle.xbrl_id, IxHeadTitle.current_period)
        .where(IxHeadTitle.securities_code == code)
        .order_by(IxHeadTitle.reporting_date.desc())
    )
    if period:
        statement = statement.where(IxHeadTitle.current_period == period)
    statement = statement.offset(count).limit(1)
    result = session.exec(statement).first()

    if result is None:
        raise HTTPException(status_code=404, detail="Item not found")

    xbrl_id = result.xbrl_id
