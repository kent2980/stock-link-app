...
def annual_securities_report_filing_date_as_planned(session: Session, head_item_key:str):
    """
    有価証券報告書提出予定日
    """
    statement = (
...
def applying_of_specific_accounting_of_the_consolidated_quarterly_financial_statements(session: Session, head_item_key:str):
    """
    ...
    statement = (
        select(IxNonNumeric)
        .where(
            IxNonNumeric.name == 'tse-ed-t_ApplyingOfSpecificAccountingOfTheConsolidatedQuarterlyFinancialStatements',
            IxNonNumeric.head_item_key == head_item_key,
            IxNonNumeric.context == context,
        )
    )
...
def applying_of_specific_accounting_of_the_consolidated_semi_annual_financial_statements(session: Session, head_item_key:str):
    """
    ...
    statement = (
        select(IxNonNumeric)
        .where(
            IxNonNumeric.name == 'tse-ed-t_ApplyingOfSpecificAccountingOfTheConsolidatedSemiAnnualFinancialStatements',
            IxNonNumeric.head_item_key == head_item_key,
            IxNonNumeric.context == context,
        )
    )
...
def applying_of_specific_accounting_of_the_quarterly_financial_statements(session: Session, head_item_key:str):
    """
    ...
    statement = (
        select(IxNonNumeric)
        .where(
            IxNonNumeric.name == 'tse-ed-t_ApplyingOfSpecificAccountingOfTheQuarterlyFinancialStatements',
            IxNonNumeric.head_item_key == head_item_key,
            IxNonNumeric.context == context,
        )
    )
...
def applying_of_specific_accounting_of_the_semi_annual_financial_statements(session: Session, head_item_key:str):
    """
    ...
    statement = (
        select(IxNonNumeric)
        .where(
            IxNonNumeric.name == 'tse-ed-t_ApplyingOfSpecificAccountingOfTheSemiAnnualFinancialStatements',
            IxNonNumeric.head_item_key == head_item_key,
            IxNonNumeric.context == context,
        )
    )
...
def changes_based_on_revisions_of_accounting_standard(session: Session, head_item_key:str):
    """
    会計基準等の改正に伴う会計方針の変更
    """
    statement = (
        select(IxNonNumeric)
        .where(
            IxNonNumeric.name == 'tse-ed-t_ChangesBasedOnRevisionsOfAccountingStandard',
            IxNonNumeric.head_item_key == head_item_key,
            IxNonNumeric.context == context,
        )
    )
...
def changes_in_accounting_estimates(session: Session, head_item_key:str):
    """
    会計上の見積りの変更
    """
    statement = (
        select(IxNonNumeric)
        .where(
            IxNonNumeric.name == 'tse-ed-t_ChangesInAccountingEstimates',
            IxNonNumeric.head_item_key == head_item_key,
            IxNonNumeric.context == context,
        )
    )
...
def changes_in_accounting_estimates_interim(session: Session, head_item_key:str):
    """
    会計上の見積りの変更、中間期
    """
    statement = (
        select(IxNonNumeric)
        .where(
            IxNonNumeric.name == 'tse-ed-t_ChangesInAccountingEstimatesInterim',
            IxNonNumeric.head_item_key == head_item_key,
            IxNonNumeric.context == context,
        )
    )
...
def changes_other_than_ones_based_on_revisions_of_accounting_standard(session: Session, head_item_key:str):
    """
    ...
    statement = (
        select(IxNonNumeric)
        .where(
            IxNonNumeric.name == 'tse-ed-t_ChangesOtherThanOnesBasedOnRevisionsOfAccountingStandard',
            IxNonNumeric.head_item_key == head_item_key,
            IxNonNumeric.context == context,
        )
    )
...
def convening_briefing_of_annual_results(session: Session, head_item_key:str):
    """
    決算説明会開催の有無
    """
    statement = (
...
def correction_of_financial_forecast_in_this_quarter(session: Session, head_item_key:str):
    """
    ...
    statement = (
        select(IxNonNumeric)
        .where(
            IxNonNumeric.name == 'tse-ed-t_CorrectionOfFinancialForecastInThisQuarter',
            IxNonNumeric.head_item_key == head_item_key,
            IxNonNumeric.context == context,
        )
    )
...
def date_of_general_shareholders_meeting_as_planned(session: Session, head_item_key:str):
    """
    定時株主総会開催予定日
    """
    statement = (
...
def material_changes_in_subsidiaries_during_this_period_changes_in_scope_of_consolidations_resulting_from_change_is_subsidiaries(session: Session, head_item_key:str):
    """
    ...
    statement = (
        select(IxNonNumeric)
        .where(
            IxNonNumeric.name == 'tse-ed-t_MaterialChangesInSubsidiariesDuringThisPeriodChangesInScopeOfConsolidationsResultingFromChangeIsSubsidiaries',
            IxNonNumeric.head_item_key == head_item_key,
            IxNonNumeric.context == context,
        )
    )
...
def name_of_subsidiaries_excluded_from_consolidation(session: Session, head_item_key:str):
    """
    社名、除外
    """
    statement = (
        select(IxNonNumeric)
        .where(
            IxNonNumeric.name == 'tse-ed-t_NameOfSubsidiariesExcludedFromConsolidation',
            IxNonNumeric.head_item_key == head_item_key,
            IxNonNumeric.context == context,
        )
    )
...
def name_of_subsidiaries_newly_consolidated(session: Session, head_item_key:str):
    """
    社名、新規
    """
    statement = (
        select(IxNonNumeric)
        .where(
            IxNonNumeric.name == 'tse-ed-t_NameOfSubsidiariesNewlyConsolidated',
            IxNonNumeric.head_item_key == head_item_key,
            IxNonNumeric.context == context,
        )
    )
...
def note_to_applying_of_specific_accounting_of_the_consolidated_quarterly_financial_statements(session: Session, head_item_key:str):
    """
    ...
    statement = (
        select(IxNonNumeric)
        .where(
            IxNonNumeric.name == 'tse-ed-t_NoteToApplyingOfSpecificAccountingOfTheConsolidatedQuarterlyFinancialStatements',
            IxNonNumeric.head_item_key == head_item_key,
            IxNonNumeric.context == context,
        )
    )
...
def note_to_applying_of_specific_accounting_of_the_consolidated_semi_annual_financial_statements(session: Session, head_item_key:str):
    """
    ...
    statement = (
        select(IxNonNumeric)
        .where(
            IxNonNumeric.name == 'tse-ed-t_NoteToApplyingOfSpecificAccountingOfTheConsolidatedSemiAnnualFinancialStatements',
            IxNonNumeric.head_item_key == head_item_key,
            IxNonNumeric.context == context,
        )
    )
...
def note_to_applying_of_specific_accounting_of_the_quarterly_financial_statements(session: Session, head_item_key:str):
    """
    ...
    statement = (
        select(IxNonNumeric)
        .where(
            IxNonNumeric.name == 'tse-ed-t_NoteToApplyingOfSpecificAccountingOfTheQuarterlyFinancialStatements',
            IxNonNumeric.head_item_key == head_item_key,
            IxNonNumeric.context == context,
        )
    )
...
def note_to_applying_of_specific_accounting_of_the_semi_annual_financial_statements(session: Session, head_item_key:str):
    """
    ...
    statement = (
        select(IxNonNumeric)
        .where(
            IxNonNumeric.name == 'tse-ed-t_NoteToApplyingOfSpecificAccountingOfTheSemiAnnualFinancialStatements',
            IxNonNumeric.head_item_key == head_item_key,
            IxNonNumeric.context == context,
        )
    )
...
def note_to_cash_flows(session: Session, head_item_key:str):
    """
    キャッシュ・フローの状況に関する注記
    """
    statement = (
...
def note_to_changes_in_accounting_policies_accounting_estimates_and_retrospective_restatement_interim_consolidated(session: Session, head_item_key:str):
    """
    ...
    statement = (
        select(IxNonNumeric)
        .where(
            IxNonNumeric.name == 'tse-ed-t_NoteToChangesInAccountingPoliciesAccountingEstimatesAndRetrospectiveRestatementInterimConsolidated',
            IxNonNumeric.head_item_key == head_item_key,
            IxNonNumeric.context == context,
        )
    )
...
def note_to_changes_in_accounting_policies_accounting_estimates_and_retrospective_restatement_quarterly_consolidated(session: Session, head_item_key:str):
    """
    ...
    statement = (
        select(IxNonNumeric)
        .where(
            IxNonNumeric.name == 'tse-ed-t_NoteToChangesInAccountingPoliciesAccountingEstimatesAndRetrospectiveRestatementQuarterlyConsolidated',
            IxNonNumeric.head_item_key == head_item_key,
            IxNonNumeric.context == context,
        )
    )
...
def note_to_changes_in_accounting_policies_and_accounting_estimates_retrospective_restatement(session: Session, head_item_key:str):
    """
    ...
    statement = (
        select(IxNonNumeric)
        .where(
            IxNonNumeric.name == 'tse-ed-t_NoteToChangesInAccountingPoliciesAndAccountingEstimatesRetrospectiveRestatement',
            IxNonNumeric.head_item_key == head_item_key,
            IxNonNumeric.context == context,
        )
    )
...
def note_to_financial_results(session: Session, head_item_key:str):
    """
    業績に関する注記
    """
    statement = (
        select(IxNonNumeric)
        .where(
            IxNonNumeric.name == 'tse-ed-t_NoteToFinancialResults',
            IxNonNumeric.head_item_key == head_item_key,
            IxNonNumeric.context == context,
        )
    )
...
def note_to_material_changes_in_subsidiaries_during_this_period(session: Session, head_item_key:str):
    """
    ...
    statement = (
        select(IxNonNumeric)
        .where(
            IxNonNumeric.name == 'tse-ed-t_NoteToMaterialChangesInSubsidiariesDuringThisPeriod',
            IxNonNumeric.head_item_key == head_item_key,
            IxNonNumeric.context == context,
        )
    )
...
def retrospective_restatement(session: Session, head_item_key:str):
    """
    修正再表示
    """
    statement = (
        select(IxNonNumeric)
        .where(
            IxNonNumeric.name == 'tse-ed-t_RetrospectiveRestatement',
            IxNonNumeric.head_item_key == head_item_key,
            IxNonNumeric.context == context,
        )
    )
...
def retrospective_restatement_interim(session: Session, head_item_key:str):
    """
    修正再表示、中間期
    """
    statement = (
        select(IxNonNumeric)
        .where(
            IxNonNumeric.name == 'tse-ed-t_RetrospectiveRestatementInterim',
            IxNonNumeric.head_item_key == head_item_key,
            IxNonNumeric.context == context,
        )
    )
...
def review_of_attached_quarterly_financial_statements_by_a_certified_public_accountant_or_an_audit_firm(session: Session, head_item_key:str):
    """
    ...
    statement = (
        select(IxNonNumeric)
        .where(
            IxNonNumeric.name == 'tse-ed-t_ReviewOfAttachedQuarterlyFinancialStatementsByACertifiedPublicAccountantOrAnAuditFirm',
            IxNonNumeric.head_item_key == head_item_key,
            IxNonNumeric.context == context,
        )
    )
...
def setting_of_specific_transaction_account_bk(session: Session, head_item_key:str):
    """
    特定取引勘定設置の有無、銀行
    """
    statement = (
        select(IxNonNumeric)
        .where(
            IxNonNumeric.name == 'tse-ed-t_SettingOfSpecificTransactionAccountBK',
            IxNonNumeric.head_item_key == head_item_key,
            IxNonNumeric.context == context,
        )
    )
...
def significant_changes_in_the_scope_of_consolidation_during_the_period(session: Session, head_item_key:str):
    """
    期中における連結範囲の重要な変更
    """
    statement = (
        select(IxNonNumeric)
        .where(
            IxNonNumeric.name == 'tse-ed-t_SignificantChangesInTheScopeOfConsolidationDuringThePeriod',
            IxNonNumeric.head_item_key == head_item_key,
            IxNonNumeric.context == context,
        )
    )
...
def supplemental_material_of_annual_results(session: Session, head_item_key:str):
    """
    決算補足説明資料作成の有無
    """
    statement = (
...
def target_audience_briefing_of_annual_results(session: Session, head_item_key:str):
    """
    対象者、決算説明会
    """
    statement = (
...
def tokyo_stock_exchange1st_section(session: Session, head_item_key:str):
    """
    東京証券取引所 第一部
    """
    statement = (
...
def tokyo_stock_exchange2nd_section(session: Session, head_item_key:str):
    """
    東京証券取引所 第二部
    """
    statement = (
...
def tokyo_stock_exchange_jasdaq(session: Session, head_item_key:str):
    """
    東京証券取引所 JASDAQ
    """
    statement = (
...
def tokyo_stock_exchange_mothers(session: Session, head_item_key:str):
    """
    東京証券取引所 マザーズ
    """
    statement = (
...
def way_of_getting_supplemental_material_of_annual_results(session: Session, head_item_key:str):
    """
    入手方法等、決算補足説明資料
    """
    statement = (
